...
1 客户端发出释放连接信号，服务端响应并进入关闭等待  --  挥手一次  ||  客户端说拜拜了您嘞，服务端回应可以哦我先把我这边事情处理完；

2 服务端发送进入关闭等待状态的确认信号，客户端接收信号进入终止等待状态 || 服务端收到信号

3 服务端将所有数据发送完毕后发送连接释放信号，客户端响应信号并发出确认，进入closed

4 服务端接收到客户端发送的确认，进入closed状态
...
栈：只能在一端进行插入和删除操作的特殊线性表。它按照先进后出的原则存储数据，
...
curl的http的header的ip真的能绕过反爬虫的检测吗

用了ras算法加密数据了https还有意义吗（为啥不直接用http）

php的fpm的拥塞机制是怎样的，排队机制是怎样的，如果激增流量的话你的web应用站点的表现是怎样的

服务器的高可用机制有了解过吗，你们站点的性能瓶颈有了解过吗
